...
<div class="flex justify-center items-center background w-full h-screen fixed top-0 left-0">
  <Card.Root class="w-1/4">
    ...
    <Card.Content>
      <Select.Root>
        ...
        <Select.Content>
          {#each restaurants as restaurant}
            <div>
              <Select.Item on:click={chooseRestaurant(restaurant.id)} value={restaurant.id} label={restaurant.name}
                >{restaurant.name}
              </Select.Item>
            </div>
          {/each}
      ...
      {#if restaurantStore}
        <Button class="mt-2 w-full" on:click={goToRestaurant}>Login</Button>
      {/if}
    </Card.Content>
  </Card.Root>
</div>
